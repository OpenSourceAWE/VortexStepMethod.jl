...
@testset "set_va! with VSMSettings applies the yaw rate about body z" begin
    ...
    try
        ...
        for p in body_aero.panels
            @test p.va ≈ expected_va .- omega × p.control_point atol=1e-10
        end
...
"""
    test_rigid_body_inflow(body_aero, va, omega, reference_point)

Test that every panel sees `va` plus the inflow of a body turning at `omega` about
`reference_point`.
"""
function test_rigid_body_inflow(body_aero, va, omega, reference_point)
    for panel in body_aero.panels
        expected_va = va .- omega × (panel.control_point .- reference_point)
        @test panel.va ≈ expected_va atol=1e-12
    end
end
...
@testset "set_va! rotates the body about reference_point" begin
    ...
    set_va!(body_aero, va, omega; reference_point)
    @test body_aero.reference_point ≈ reference_point
    test_rigid_body_inflow(body_aero, va, omega, reference_point)

    body_aero.omega = 2 .* omega
    test_rigid_body_inflow(body_aero, va, 2 .* omega, reference_point)

    reinit!(body_aero; va, omega)
    test_rigid_body_inflow(body_aero, va, omega, reference_point)

    body_aero.reference_point = zeros(3)
    test_rigid_body_inflow(body_aero, va, omega, zeros(3))
end
