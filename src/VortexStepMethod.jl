...
export plot_airfoil_slices, plot_airfoils, plot_combined_analysis,
    plot_distribution, plot_geometry, plot_polar_data, plot_polars,
    plot_section_polars, save_plot, show_plot

# Plotting functions live in the `VortexStepMethodMakieExt` extension, loaded
# once a Makie backend (`GLMakie` or `CairoMakie`) and `MakieControlPlots` are
# available. The declarations below carry the public docstrings; the extension
...
# Keyword arguments
- `data_type`: file extension for saving (default: `.pdf` for CairoMakie, `.png` otherwise)
...
"""
    plot_airfoil_slices(obj_path::String; kwargs...)

Plot airfoil cross-sections extracted from an OBJ mesh with Kulfan CST fits overlaid.
...
every section of a wing using stored `POLAR_VECTORS` data.
...
coordinates, so the YAML is the source).
